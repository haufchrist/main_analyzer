def mainreducer(delaysperscan, pointsperdelay, intervallwidth, maxphotons, stackingthreshold, delayzero, sampledirectory, voltoffset, voldifference):
    ...
    datfilesimp = [f for f in os.listdir(sampledirectory) if os.path.isfile(os.path.join(sampledirectory, f))]
    ...
    for h in range(0, len(datfiles)):
        ...
        filename = (sampledirectory + datfiles[h])
        start = time.clock()
        ...
        print('Scan ' + filenameparts[4] + '\tprocessed in ' + str(round((end-start),3)) + ' Seconds;', str(len(datfiles)-h-1)  + ' files remaining')              
